  ...
  class magicLine {
    constructor(node, settings) {
      ...
      const setActiveElement = (links, event) => {
        ...
        if (event.target.classList[0] == 'dropdown-link') {
          let toolLink = document.getElementById('tool-link');
          curEl = toolLink;
        } else {
          curEl = event.target;
      ...
      const drawLine = (line, active, init) => {
        const navWidth = document.getElementById('navbar-container').offsetWidth;
        const reduceBy = ($(window).width() - navWidth) / 2;
        let lineLeft = active.rect.left - reduceBy - 15;
      ...
      const onLoad = () => {
        Array.prototype.forEach.call(this.elements, el => {
          ...
          linkWrapper.id = "magicLineInner";

          let magicLineEl = document.createElement("div");
          magicLineEl.className = this.settings.lineClass;
          magicLineEl.id = this.settings.lineId;
          document.getElementById('navbar-container').classList.add('magic-line-active');
          ...
          setTimeout(() => {
            let initActive = document.getElementsByClassName("is-active");
            if (initActive !== null && initActive !== undefined && initActive[0] !== undefined) {
              const data = {
                el: initActive[0],
                rect: initActive[0].getBoundingClientRect(),
              };
              initActive[0].classList.add("active");
              setTimeout(() => {
                drawLine(magicLineEl, data, true);
              });
            }
          });
        });
